...
function chatItem(data, optFunc, replyFunc, moveToMsg, itemHeight) {
    return (
        <View style={[{
            justifyContent: data.me ? 'flex-end' : 'flex-start'
        }, styles.chatContainer]}
            onLayout={object => itemHeight(object.nativeEvent.layout.height)}>
            {data.me ? null :
                <Animated.View style={{
                    width: 40, marginRight: data.margin, flexDirection: 'row',
                    opacity: data.opacity, justifyContent: 'center', alignItems: 'center'
                }}>
...
export default class ChatRoomComponent extends React.Component {
    constructor(props) {
        super(props)
        this.state = {
            username: '',
                ...
                <View style={[styles.textBoxContainer, constStyle.shadow.depth2]}>
                    {this.state.replyData ?
                        <Animated.View style={[{
                            ...
                            padding: 10, width: '100%', borderRadius: 50 / 2,
                            opacity: this.state.replyDialog.opacity, flexDirection: 'row',
                            marginBottom: 5
                        }]}>
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <TextInput style={[styles.textBox]} placeholder="Type something..."
                            placeholderTextColor='grey' multiline={true}
                            value={this.state.chat}
                            onFocus={() => this._showOptAnim(false, this.state.optData)}
